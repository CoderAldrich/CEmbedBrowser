...
HRESULT STDMETHODCALLTYPE UI_GetHostInfo(IDocHostUIHandler FAR* This, DOCHOSTUIINFO __RPC_FAR *pInfo)
{
	...
	pInfo->dwFlags = DOCHOSTUIFLAG_NO3DBORDER | 
						...
						DOCHOSTUIFLAG_NO3DOUTERBORDER;// wlq:去除滚动条
 ...
 */
// 按钮id
#define IDB_CLOSE     3301

LPCSTR WINDOWS_CLASS = "MyClass";    //类名  
LPCSTR WINDOWS_TITLE = "测试按钮";   //窗口标题 
HINSTANCE Btn_Instance;
...
LRESULT CALLBACK Btn_Close_Proc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		{
			...
			hBmp = LoadBitmapA((HINSTANCE)Btn_Instance, MAKEINTRESOURCE(IDB_BITMAP1)); // 资源添加方式：右键Resource Files->添加->资源->bitmap->导入本地位图文件，编译器自动产生资源ID
			SelectObject(hMemDC, hBmp);
			TransparentBlt(hdc, 0, 0, rect.right - rect.left, rect.bottom - rect.top, hMemDC, 0, 0, 40, 40, RGB(0, 0, 0));
			BitBlt(hdc, 0, 0, 15, 15, hMemDC, 0, 0, SRCCOPY);
			DeleteObject(hMemDC);
			DeleteDC(hdc);
			EndPaint(hwnd, &ps);
			break;
	...
	case WM_MOUSEHOVER:		// 鼠标悬停hover特效
		{
			//MessageBoxA(hwnd, "hover", "note", MB_OK);

			// 鼠标移上来时切换位图的方法展示hover效果
			MoveWindow(hwnd, 305, 10, 20, 20, TRUE);
			InvalidateRect(hwnd, &rect, TRUE);
			hdc = BeginPaint(hwnd, &ps);
			hMemDC = CreateCompatibleDC(hdc);
			hBmp = LoadBitmapA((HINSTANCE)Btn_Instance, MAKEINTRESOURCE(IDB_BITMAP2));
			SelectObject(hMemDC, hBmp);
			TransparentBlt(hdc, 0, 0, rect.right - rect.left, rect.bottom - rect.top, hMemDC, 0, 0, 40, 40, RGB(0, 0, 0));
			BitBlt(hdc, 0, 0, 20, 20, hMemDC, 0, 0, SRCCOPY);
			DeleteObject(hMemDC);
			DeleteDC(hdc);
			EndPaint(hwnd, &ps);
	...
	case WM_MOUSELEAVE:		// 鼠标移走
		{
			//MessageBoxA(hwnd, "leave", "note", MB_OK);

			// 鼠标移走，按钮复原
			MoveWindow(hwnd, 305, 10, 15, 15, TRUE);
			InvalidateRect(hwnd, &rect, TRUE);
			hdc = BeginPaint(hwnd, &ps);
			hMemDC = CreateCompatibleDC(hdc);
			hBmp = LoadBitmapA((HINSTANCE)Btn_Instance, MAKEINTRESOURCE(IDB_BITMAP1));
			SelectObject(hMemDC, hBmp);
			TransparentBlt(hdc, 0, 0, rect.right - rect.left, rect.bottom - rect.top, hMemDC, 0, 0, 40, 40, RGB(0, 0, 0));
			BitBlt(hdc, 0, 0, 15, 15, hMemDC, 0, 0, SRCCOPY);
			DeleteObject(hMemDC);
	...
	case WM_CREATE:
		{
			...
			hBtnClose = CreateWindowA("Button", "", WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON/* 必须是子窗口，方便布局，同时也是点击按钮 */,  
				305, 10, 15, 15, hwnd, (HMENU)IDB_CLOSE/* 设置的窗口按钮ID */, ((LPCREATESTRUCT)lParam)->hInstance, NULL); 
			
			SetWindowLongA(hBtnClose, GWL_WNDPROC, (long)Btn_Close_Proc);// 窗口子类化，重新指定子窗口的回调函数，同样使用该函数还可以更改窗口风格等
			SetWindowPos(hBtnClose, HWND_TOP, 0, 0, 0, 0, SWP_SHOWWINDOW | SWP_NOMOVE | SWP_NOSIZE);

			// 去掉边框，先获取当前窗口的风格值，然后使用异或操作去掉边框相关的属性值
			dwStyle = GetWindowLong(hwnd, GWL_STYLE);
			dwNewStyle = (dwStyle & ~WS_BORDER);
			SetWindowLong(hwnd, GWL_STYLE, dwNewStyle);
			dwExStyle = GetWindowLong(hwnd, GWL_EXSTYLE);
			dwNewExStyle = (dwExStyle & ~(WS_EX_DLGMODALFRAME | WS_EX_WINDOWEDGE));
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hInstNULL, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	if (OleInitialize(NULL) == S_OK)
	{
		...
		wc.lpfnWndProc = WindowProc;	// 创建、显示子窗口的动作通常在父窗口的WM_CREATE消息响应中
		wc.lpszClassName = &ClassName[0];
		RegisterClassEx(&wc);

		// 创建主体窗口，用来显示html网页
		if ((msg.hwnd = CreateWindowEx(WS_EX_TOOLWINDOW/* 该属性决定窗口没有顶部菜单栏 */, &ClassName[0], "ads_pop_wnd", 
			WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_POPUP /* 该属性决定窗口没有自带的滚动条和边框 */,
			100/*x*/, 100/*y*/, 330/*with*/, 254/*height*/,
			NULL, NULL, hInstance, 0)))
		{
			// 使用IWebBrowser2类显示网页内容，去掉网页自带滚动条属性的操作在函数 UI_GetHostInfo 中
			// For this window, display a URL. This could also be a HTML file on disk such as "c:\\myfile.htm".
			// DisplayHTMLPage(msg.hwnd, "http://pic.zuhaowan.com/images/gg_img/2019-11-14/5dccc15a33069.jpg");
			DisplayHTMLPage(msg.hwnd, "www.hao123.com");
